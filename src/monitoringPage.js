...
async function loadMonitoring(silent = false) {
  ...
  try {
    ...
    if (!silent) renderFromCache(); else renderFromCache();
  } catch (err) {
...
function renderTable(students) {
  ...
  box.innerHTML = `
    <table class="mon-table">
      <thead><tr><th>No</th><th>Nama</th><th>Kelas</th><th>NIS</th><th>Status Masuk</th><th>Jam Masuk</th><th>Pulang</th><th>Jam Pulang</th><th>Aksi WA</th></tr></thead>
      <tbody>${students.map((s, i) => `
        <tr class="${!s.jam_masuk && !s.jam_tidak_hadir ? "mon-row-danger" : ""}">
          ...
          <td>${!s.jam_masuk && !s.jam_tidak_hadir ? `
            <button class="mon-btn-wa mon-wa-st" data-student="${escapeHtml(s.student_id)}" data-name="${escapeHtml(s.student_name)}" data-type="sangat_terlambat">📩 Hadir (Sangat Terlambat)</button>
            <button class="mon-btn-wa mon-wa-th" data-student="${escapeHtml(s.student_id)}" data-name="${escapeHtml(s.student_name)}" data-type="tidak_hadir">📩 Tidak Hadir</button>` : "-"}</td>
        </tr>`).join("")}
      </tbody>
    </table>`;
}
...
function injectMonitoringStyles() {
  ...
  style.textContent = `
    .mon-toolbar{display:flex;gap:12px;flex-wrap:wrap;align-items:end;margin-bottom:16px;background:#f8fafc;padding:14px;border-radius:12px;border:1px solid #e2e8f0;}
    ...
    .mon-ok{background:#dcfce7;color:#166534;} .mon-warn{background:#fef9c3;color:#854d0e;} .mon-bad{background:#fee2e2;color:#991b1b;}
    ...
    .mon-wa-st{background:#16a34a;} .mon-wa-th{background:#dc2626;}
