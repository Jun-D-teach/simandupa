...
async function initSettingsPage() {
  ...
  // ==========================================
  // 1. LOAD SETTINGS (Membaca data dari DB)
  // ==========================================
  try {
    ...
    if (result.success) {
      ...
      // ✅ BARU: Load Waktu Kirim Notifikasi WA
      setValueIfExists("absent_notify_time", data.absent_notify_time); 
    } else {
  ...
  // ==========================================
  // 2. SAVE SETTINGS (Menyimpan data ke DB)
  // ==========================================
  saveBtn.addEventListener("click", async () => {
    try {
      ...
      // ✅ BARU: Ambil nilai Waktu Kirim Notifikasi WA
      const absent_notify_time = getValueIfExists("absent_notify_time"); 

      // Masukkan ke payload jika nilainya ada
      ...
      if (school_logo_url !== undefined) payload.school_logo_url = school_logo_url;
      ...
      if (default_homeroom_name !== undefined) payload.default_homeroom_name = default_homeroom_name;
      if (default_homeroom_nip !== undefined) payload.default_homeroom_nip = default_homeroom_nip;
      if (current_semester !== undefined) payload.current_semester = current_semester;
      if (current_school_year !== undefined) payload.current_school_year = current_school_year;
      
      if (school_start_time !== undefined) payload.school_start_time = school_start_time;
      if (school_late_time !== undefined) payload.school_late_time = school_late_time;
      if (school_return_time !== undefined) payload.school_return_time = school_return_time;
      if (attendance_open_time !== undefined) payload.attendance_open_time = attendance_open_time;
      if (attendance_close_return_time !== undefined) payload.attendance_close_return_time = attendance_close_return_time;
      
      // ✅ BARU: Masukkan ke payload
      if (absent_notify_time !== undefined) payload.absent_notify_time = absent_notify_time; 
      ...
      // Update localStorage untuk info akademik
      if (payload.current_semester !== undefined) {
        localStorage.setItem("simCurrentSemester", payload.current_semester || "-");
      }
      if (payload.current_school_year !== undefined) {
        localStorage.setItem("simCurrentSchoolYear", payload.current_school_year || "-");
      }
